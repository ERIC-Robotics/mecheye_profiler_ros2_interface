...
{
    auto* mechMindProfiler = static_cast<MechMindProfiler2*>(pUser);
    if (mechMindProfiler)
        mechMindProfiler->handleCallbackBatch(batch);
}
} // namespace

MechMindProfiler2::MechMindProfiler2()
{
    node = rclcpp::Node::make_shared("mechmind_profiler_publisher_service_2");

    node->declare_parameter<std::string>("profiler_ip", "192.168.0.38");
    ...
    pub_intensity =
        node->create_publisher<sensor_msgs::msg::Image>("/mechmind_profiler_2/intensity_image", 1);
    pub_depth = node->create_publisher<sensor_msgs::msg::Image>("/mechmind_profiler_2/depth_map", 1);
    pub_pcl =
        node->create_publisher<sensor_msgs::msg::PointCloud2>("/mechmind_profiler_2/point_cloud", 1);
