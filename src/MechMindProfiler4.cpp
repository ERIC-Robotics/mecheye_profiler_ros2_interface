...
{
    auto* mechMindProfiler = static_cast<MechMindProfiler4*>(pUser);
    if (mechMindProfiler)
        mechMindProfiler->handleCallbackBatch(batch);
}
} // namespace

MechMindProfiler4::MechMindProfiler4()
{
    node = rclcpp::Node::make_shared("mechmind_profiler_publisher_service_4");

    node->declare_parameter<std::string>("profiler_ip", "192.168.0.40");
    ...
    pub_intensity =
        node->create_publisher<sensor_msgs::msg::Image>("/mechmind_profiler_4/intensity_image", 1);
    pub_depth = node->create_publisher<sensor_msgs::msg::Image>("/mechmind_profiler_4/depth_map", 1);
    pub_pcl =
        node->create_publisher<sensor_msgs::msg::PointCloud2>("/mechmind_profiler_4/point_cloud", 1);
